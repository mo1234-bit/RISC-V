module execute_cycle(
    ...
    // Sub-word memory: funct3 forwarded to MEM stage for byte_en / sign-extension
    output [2:0] funct3M,
    ...
    // ----------------------------
    wire [31:0] Src_A_ALU, Src_B_ALU;
    wire [31:0] Src_A_FPU, Src_B_FPU, Src_B_FPU_interim, Src_B_ALU_interim;
    ...
    Mux_3_by_1 alu_srca_mux (
        .a(RD1_E), .b(ResultW), .c(ALU_ResultM),
        .s(ForwardA_E), .d(Src_A_ALU)
    );

    Mux_3_by_1 alu_srcb_mux (
        .a(RD2_E), .b(ResultW), .c(ALU_ResultM),
        .s(ForwardB_E), .d(Src_B_ALU_interim)
    );

    Mux alu_src_mux (
        .a(Src_B_ALU_interim), .b(Imm_Ext_E),
        .s(ALUSrcE || FResultSrcE), .c(Src_B_ALU)
    );
    ...
    Mux_3_by_1 fpu_srca_mux (
        .a(FRD1_E), .b(FPU_ResultW), .c(FPU_ResultEM),
        .s(ForwardA_E_FPU), .d(Src_A_FPU)
    );

    Mux_3_by_1 fpu_srcb_mux (
        .a(FRD2_E), .b(FPU_ResultW), .c(FPU_ResultEM),
        .s(ForwardB_E_FPU), .d(Src_B_FPU_interim)
    );

    Mux fpu_src_mux (
        .a(Src_B_FPU_interim), .b(Imm_Ext_E),
        .s(ALUSrcE), .c(Src_B_FPU)
    );
    ...
    PC_Adder branch_adder (
        .a(PCE), .b(Imm_Ext_E), .c(BranchTarget)
    );

    assign stall_f = ((fsubE && !finish_adder) || (faddE && !finish_adder) ||
                      (fmulE && !finish_mul)   || (fsqrtE && !finish_sqr)  ||
                      (fdivE && !finish_div)) ? 1'b1 : stall_f_buf;
    ...
    assign JumpTarget = (OpE == 7'b1100111) ? ((Src_A_ALU + Imm_Ext_E) & ~32'h1)
                                            : (PCE + Imm_Ext_E);

    // ----------------------------
    // Branch condition logic
    // ----------------------------
    wire signed_less   = (NegativeE ^ OverFlowE);
    wire unsigned_less = ~CarryE;
